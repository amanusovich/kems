...
public class IPLHtmlExporter {
    ...
    private static void appendD3Script(StringBuilder sb) {
        ...
        sb.append("      if (selectedBox) d3.select(selectedBox).classed('selected', false);\n");
        ...
        sb.append("      grp.append('text')\n");
        sb.append("        .attr('x', PAD)\n");
        sb.append("        .attr('y', HDR_H + PAD + ROW_H * i + ROW_H - 3)\n");
        sb.append("        .style('font-size', '9px').style('fill', nodeColor(f)).style('cursor', 'pointer')\n");
        sb.append("        .text(f.text)\n");
        sb.append("        .on('click', (event) => {\n");
        sb.append("          event.stopPropagation();\n");
        sb.append("          if (selectedBox) d3.select(selectedBox).classed('selected', false);\n");
        ...
        sb.append("  (branchData.bStarExtensions || []).forEach(e => {\n");
        sb.append("    const li = document.createElement('li');\n");
        sb.append("    li.textContent = e;\n");
        sb.append("    bstarList.appendChild(li);\n");
        sb.append("  });\n");
        sb.append("  if (!branchData.bStarExtensions || !branchData.bStarExtensions.length) {\n");
        sb.append("    bstarList.innerHTML = '<li style=\"color:#aaa\">None</li>';\n");
        sb.append("  }\n\n");
        sb.append("  const riList = document.getElementById('detail-rinstances');\n");
        sb.append("  riList.innerHTML = '';\n");
        sb.append("  (branchData.rinstances || []).forEach(r => {\n");
        sb.append("    const li = document.createElement('li');\n");
        sb.append("    li.textContent = r;\n");
        sb.append("    riList.appendChild(li);\n");
        sb.append("  });\n");
        sb.append("  if (!branchData.rinstances || !branchData.rinstances.length) {\n");
        sb.append("    riList.innerHTML = '<li style=\"color:#aaa\">None</li>';\n");
        sb.append("  }\n");
        sb.append("}\n\n");
        ...
        sb.append("  div.textContent = DATA.trace.events.map((e,i) =>\n");
        sb.append("    'Step ' + (i+1) + ': [' + e.type + '] ' + (e.description || '') +\n");
        sb.append("    (e.branch ? ' @' + e.branch : '')\n");
        sb.append("  ).join('\\n');\n");
        sb.append("}\n\n");
